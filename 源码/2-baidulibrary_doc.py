...
class BaiduLibrary:
    def __init__(self, url):
        self.url = url
        self.param_url = "https://wenku.baidu.com/browse/getrequest?doc_id={}&type=html&rn=50&pn=1"
    ...
    @staticmethod
    def get_data_url_list(param_str):  # 提取所有保存有数据的url
    ...
    # def get_data(self, data_str):  # 提取数据  例子：https://wenku.baidu.com/view/da49031bcc7931b765ce153f.html
    ...
    # def get_data(self, data_str):  # 提取数据  例子：https://wenku.baidu.com/view/64835d0e28ea81c758f578e6.html
    #     data_str = self.data_re.findall(data_str)[0]
    #     body = json.loads(data_str)['body']
    #     content = ""
    #
    ...
    def get_data(self, data_str):  # 提取数据  例子：https://wenku.baidu.com/view/8e2373ae71fe910ef12df87d?pcf=2&from=singlemessage
        data_str = self.data_re.findall(data_str)[0]
        body = json.loads(data_str)['body']
        content = ""
        ...
        for index, item in enumerate(body):
            if not isinstance(item['c'], dict) and item['c'] != '':
                ...
                content, is_line_feed = self.add_space(is_line_feed, last_y, current_y, next_y, item, next_c, index, content)
    ...
    def add_space(self, is_line_feed, last_y, current_y, next_y, item, next_c, index, content):  # 添加空格，缩进
        if not ((next_c == ' ' and item['c'] == '') or (next_c == '' and item['c'] == ' ')):  # abs(current_y-next_y) 加粗的字体y轴可能有多少偏差
            if current_y == next_y or abs(float(current_y)-float(next_y)) <= 4:    # 确保是 上一个内容与当前内容 同一行
                content, is_line_feed = self.do_add_space(is_line_feed, item, index, content)
    ...
    def do_add_space(self, is_line_feed, item, index, content):  # 添加空格，缩进
        # 同一行中只有开头才需要添加缩进
        if is_line_feed:
    ...
    def run(self):
        ...
        url_list = self.get_data_url_list(param_str)
